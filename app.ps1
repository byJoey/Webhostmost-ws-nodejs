...
# Version: 2.3 - Updated Author Information
...
Write-Host "核心功能由 eooce 和 qwer-search 提供。" -ForegroundColor Magenta
...
$appJsUrl = "https://raw.githubusercontent.com/qwer-search/Webhostmost-ws-nodejs/main/app.js"
...
function Invoke-BasicConfiguration {
    Write-Host "`n--- 正在配置基本部署参数 (UUID, Domain, Port) ---" -ForegroundColor Yellow
    ...
    return @{Domain = $domain; UUID = $uuid; Port = $vl_port}
}
...
try {
    ...
    if ($null -ne $basicConfigDetails) {
        ...
        $subLink = "https://$($basicConfigDetails.Domain)/sub"
